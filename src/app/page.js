...
export default function Home() {
  ...
  const [almo, setAlmo] = useState(() => {
    if (typeof window === "undefined") return "";
    return localStorage.getItem(ALMO_STORAGE_KEY) || "";
  });
  ...
  useEffect(() => {
    ...
    const onContextMenu = (e) => {
      if (e.altKey) {
        e.preventDefault(); // bloqueia menu do botão direito
      }
    };
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="w-screen h-screen overflow-auto relative select-none mt-[50px]"
      >
